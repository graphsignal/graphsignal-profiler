...
class NVMLRecorderTest(unittest.TestCase):
    ...
    def test_record(self):
        ...
        metric_tags = graphsignal.sdk.sdk().tags()
        ...
        if key in store._metrics:
            self.assertTrue(find_last_datapoint(store, key).gauge > 0)
        key = store.metric_key('gpu.power.limit', metric_tags)
        if key in store._metrics:
            self.assertTrue(find_last_datapoint(store, key).gauge > 0)
        key = store.metric_key('gpu.clock.sm', metric_tags)
        if key in store._metrics:
            self.assertTrue(find_last_datapoint(store, key).gauge > 0)
        key = store.metric_key('gpu.clock.mem', metric_tags)
        if key in store._metrics:
            self.assertTrue(find_last_datapoint(store, key).gauge > 0)
        key = store.metric_key('gpu.clock.sm.max', metric_tags)
        if key in store._metrics:
            self.assertTrue(find_last_datapoint(store, key).gauge > 0)
        key = store.metric_key('gpu.throttle_reasons', metric_tags)
        if key in store._metrics:
            self.assertTrue(find_last_datapoint(store, key).gauge >= 0)
